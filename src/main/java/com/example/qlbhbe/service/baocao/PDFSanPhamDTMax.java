...
public class PDFSanPhamDTMax {
    ...
    public Map<String, String> exportDoanhThuPDF(SanPhamDTO sanPhamDTO) throws Exception {
        try {
            Document document = new Document();
            String FILE = "D:/BaoCaoXuatKho" + System.currentTimeMillis() + ".pdf";
...
//    }
    ...
    private void addTitlePage(Document document, SanPhamDTO sanPhamDTO)
        ...
        addEmptyLine(preface, 1);
        setRow(preface, "BÁO CÁO HÀNG XUẤT KHO", font(18, Font.BOLD), Element.ALIGN_CENTER);
    ...
    private void createTable(Paragraph paragraph, SanPhamDTO sanPhamDTO)
        ...
        setHeaderTable(table, "Giá", Element.ALIGN_CENTER, c1);
        setHeaderTable(table, "Thành Tiền", Element.ALIGN_CENTER, c1);
        setHeaderTable(table, "Cửa Hàng", Element.ALIGN_CENTER, c1);
        table.setHeaderRows(1);
        List<SanPhamDTO> sanPhamDTOList = sanPhamReport.getSanPhamDoanhThuMax(sanPhamDTO);
        for (SanPhamDTO sanPhamDTO1 : sanPhamDTOList) {
            ...
            setContentTable(table, String.valueOf(sanPhamDTO1.getSoLuongBan().intValue()), Element.ALIGN_CENTER, c1);
            setContentTable(table, String.valueOf(sanPhamDTO1.getGiaBan().intValue()), Element.ALIGN_CENTER, c1);
            setContentTable(table, String.valueOf(sanPhamDTO1.getTotalDoanhThu().intValue()), Element.ALIGN_CENTER, c1);
