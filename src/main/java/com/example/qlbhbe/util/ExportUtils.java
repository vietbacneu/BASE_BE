...
import com.viettel.pmp.partner.common.constant.ExcelColumn;
import com.viettel.pmp.partner.common.constant.ExcelTitle;
...
public class ExportUtils {
    ...
    private Row createFileTitle(int startRow, int startCol, ExcelTitle excelTitle, Workbook workbook, Sheet sheet, short rowHeight, int numCol) {
        ...
        if (excelTitle != null) {
            if (!com.viettel.pmp.partner.common.util.DataUtil.isNullOrEmpty(excelTitle.getTitle())) {
            ...
            if (!com.viettel.pmp.partner.common.util.DataUtil.isNullOrEmpty(excelTitle.getDateExportPattern()) && !com.viettel.pmp.partner.common.util.DataUtil.isNullOrEmpty(excelTitle.getDateExportTitle())) {
                Row rowDateExport = sheet.createRow(rowTitle + 1);
                Cell mainCellTitle = rowDateExport.createCell((numCol / 2)+ 1);
                mainCellTitle.setCellValue(excelTitle.getDateExportTitle() + " : " + com.viettel.pmp.partner.common.util.DataUtil.dateToString(new Date(), excelTitle.getDateExportPattern()));
    ...
    private void writeDataReport(List<ExcelColumn> lstColumn, List<?> lstData, int startRow, int startCol, Sheet sheet,
        ...
        if (lstData != null && !lstData.isEmpty()) {
            ...
            for (ExcelColumn column : lstColumn) {
                ...
                Field[] fs = com.viettel.pmp.partner.common.util.ReflectorUtil.getAllFields(firstRow.getClass());
...
//        cellStyleTitle.setFillPattern(FillPatternType.SOLID_FOREGROUND);
    ...
    public ByteArrayInputStream onExport2Table(List<ExcelColumn> lstColumn, List<ExcelColumn> lstColumn1, List<?> lstData, List<?> lstData1, int startRow, int startCol, ExcelTitle excelTitle, Boolean displayIndex,
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            mainCellTitle.setCellStyle(cellStyleTitle);
            if (!com.viettel.pmp.partner.common.util.DataUtil.isNullOrEmpty(lstColumn1)) {
